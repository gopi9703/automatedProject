...
import FileColumnForm from "./Form";

const FileColumn: React.FC = () => {
  ...
  const renderActions = (rowData: IFileColumn) => {
    return (
      <>
  ...
  const editRowData = (row: IFileColumn) => {
    setEditObj(row);
    setUserDialog(true);
    setDialogTitle("Edit File Column");
    setFormType(0);
  };

  const addNewUser = () => {
    setDialogTitle("Add File Column");
    setUserDialog(true);
    setEditObj({});
    setFormType(1);
  };
